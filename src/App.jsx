...
export default function App() {
  ...
  return <div className="site-shell">
    <header className="hero"><nav className="toolbar container" aria-label="Hauptnavigation"><div className="brand"><div><strong>NEA <em>Baukonzepte</em></strong><span>{text.tagline}</span></div></div><div className="controls"><div className="language-control" aria-label="Sprachauswahl">{['de', 'sr'].map(code => <button type="button" className={language === code ? 'active' : ''} onClick={() => setLanguage(code)} key={code} lang={code}>{code.toUpperCase()}</button>)}</div><button className="theme-button" type="button" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label="Darstellung ändern">{theme === 'dark' ? '☀' : '☾'}</button></div></nav><div className="hero-content container"><p className="eyebrow">{text.category}</p><h1>{text.heading}</h1><p className="hero-description">{text.description}</p><button className="button button-primary" type="button" onClick={requestQuote}>{text.primaryAction}</button><img className="hero-image" src="/fiberOptic.jpg" width="1600" height="900" alt="Glasfaser Tiefbau Arbeiten und LWL-Kabelverlegung für den FTTH Hausanschluss" /></div></header>
